...
- how to install `pynxtools-apm`
- how to install `pynxtools-apm` together with NOMAD
...
=== "uv"
    ...
    ```bash
    uv pip install git+https://github.com/FAIRmat-NFDI/pynxtools-apm.git
    ```

=== "pip"

    ```bash
    pip install git+https://github.com/FAIRmat-NFDI/pynxtools-apm.git
    ```

### How to install `pynxtools-apm` with NOMAD

To use `pynxtools-apm` with NOMAD, simply install it in the same environment as the `nomad-lab` package. NOMAD will recognize `pynxtools-apm` as a plugin automatically. In addition, NOMAD will install a schema for NeXus application definitions.

## Start using `pynxtools-apm`

That's it! You can now use `pynxtools-apm`!
